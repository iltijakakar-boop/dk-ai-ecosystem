...
class DateTimeTool(BaseTool):
    ...
    def _parse_offset(self, offset_str: str) -> timedelta:
        ...
        if not match:
            raise ValueError(f"Invalid timezone offset format: '{offset_str}'. Use '+HH:MM' or '-HH:MM'.")
        ...
        try:
            if action == "current_time":
                ...
                if not datetime_str or not target_offset_str:
                    return {"success": False, "error": "Parameters 'datetime_str' and 'target_offset' are required for conversion."}
