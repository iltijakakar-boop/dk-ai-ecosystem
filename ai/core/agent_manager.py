...
class AgentManager:
    ...
    def discover_agents(self) -> None:
            ...
            if entry.is_dir() and not entry.name.startswith((".", "__")):
                ...
                if os.path.exists(manifest_path) and os.path.exists(agent_file):
                    try:
                        ...
                        if os.path.exists(tools_file):
                            ...
                            try:
                                if tools_module_name in sys.modules:
                                    tools_module = importlib.reload(
                                        sys.modules[tools_module_name]
                                    )
                                else:
                                ...
                                logger.exception(
                                    f"Failed to load tools module for {agent_id}:"
                                )
                        ...
                        logger.exception(
                            f"Error loading agent from directory {entry.name}:"
                        )
        ...
        try:
            ...
            try:
                for agent_id, agent_instance in self.agents.items():
                    ...
                    if db_agent:
                        ...
                        db_agent.provider = (
                            manifest.get("provider") or settings.AI_PROVIDER
                        )
                        db_agent.status = (
                ...
                db.commit()
                logger.info(
                    "Synchronized dynamic agents with the AgentRegistry database table."
                )
            ...
            logger.warning(
                f"Database session not available for AgentRegistry sync: {e}"
            )
        ...
        return [
            {
                ...
                "provider": (
                    agent.provider.__class__.__name__ if agent.provider else None
                ),
                "tools": [t.name for t in agent.available_tools],
                "version": self.manifests.get(agent.agent_id, {}).get(
                    "version", "1.0.0"
                ),
            }
