...
def get_tool_metrics() -> Dict[str, Any]:
    ...
    try:
        ...
        successes = db.query(func.count(ToolExecutionLog.id)).filter(ToolExecutionLog.status == "success").scalar() or 0
        failures = total - successes
        
        avg_duration = db.query(func.avg(ToolExecutionLog.duration_ms)).scalar() or 0.0
        
        # Count logs with timeout error signatures
        timeouts = db.query(func.count(ToolExecutionLog.id)).filter(
            ToolExecutionLog.error.like("%timeout%")
        ).scalar() or 0
