...
class Workflow(Base):
    ...
    )  # Logical identifier across versions
    version = Column(Integer, default=1, nullable=False)
    is_active = Column(Boolean, default=True, nullable=False)
    is_template = Column(
        Boolean, default=False, nullable=False
    )  # Distinguishes seeded templates
    name = Column(String, nullable=False)
    description = Column(String, nullable=True)
    definition = Column(
        String, nullable=False
    )  # JSON-serialized steps and routing rules
...
class WorkflowExecution(Base):
    ...
    )  # JSON-serialized shared variables context
...
class TaskExecution(Base):
    ...
    task = relationship(
        "Task",
        backref=backref(
            "task_executions", cascade="all, delete-orphan", passive_deletes=True
        ),
    )
...
class WorkflowLog(Base):
    ...
    event_type = Column(
        String, nullable=False
    )  # AgentStarted, AgentFinished, TaskCreated, TaskCompleted, etc.
...
class DeadLetterQueue(Base):
    ...
    task = relationship(
        "Task",
        backref=backref(
            "dlq_entries", cascade="all, delete-orphan", passive_deletes=True
        ),
    )
    execution = relationship(
        "WorkflowExecution",
