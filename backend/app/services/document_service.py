...
class DocumentService:
    ...
    def validate_file(self, filename: str, file_size: int) -> Tuple[bool, Optional[str]]:
        ...
        if file_size > max_bytes:
            return False, f"File exceeds maximum allowed size of {settings.MAX_DOCUMENT_SIZE_MB}MB."

        # 2. Extension Validation
        ext = os.path.splitext(filename)[1].lower().strip(".")
        if ext not in settings.ALLOWED_DOCUMENT_TYPES:
            return False, f"Unsupported file type '{ext}'. Allowed types: {settings.ALLOWED_DOCUMENT_TYPES}."
    ...
    def delete_file(self, filename: str) -> bool:
        ...
        if not os.path.abspath(target_path).startswith(self.storage_path):
            logger.warning(f"Path traversal attempt prevented during file deletion: {filename}")
            return False
