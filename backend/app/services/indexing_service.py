...
class IndexingService:
    ...
    def index_document_in_background(self, document_id: int) -> None:
        ...
        if not doc:
            logger.error(f"Ingestion failed: Document ID {document_id} not found in DB.")
            db.close()
            return
        ...
        try:
            ...
            for index, rc in enumerate(raw_chunks):
                ...
                # Check Embedding Cache: see if this exact text exists in ANY chunk in database
                cached_chunk = db.query(DocumentChunk).filter(DocumentChunk.text == chunk_text).first()
                if cached_chunk and cached_chunk.embedding:
                    ...
                    db.add(new_emb)
                    logger.info(f"Embedding Cache Hit: Reusing cached vector for chunk {index}.")
                else:
                    # Needs new embedding
                    new_chunk = DocumentChunk(
                        document_id=doc.id,
            ...
            if texts_to_embed:
                logger.info(f"Generating embeddings for {len(texts_to_embed)} new chunks in batches...")
                generated_vectors = embedding_service.get_embeddings_with_retry(texts_to_embed)
            ...
            logger.info(f"Indexing completed successfully for Document: {doc.original_filename} (ID: {doc.id})")

        except Exception as e:
            db.rollback()
            logger.exception(f"Ingestion pipeline failed for Document ID {document_id}:")
    ...
    def reindex_document(self, document_id: int) -> None:
        ...
        try:
            ...
            old_chunks = db.query(DocumentChunk).filter(DocumentChunk.document_id == doc.id).all()
            ...
            for idx, nc in enumerate(new_chunks_data):
                ...
                if chunk_text in old_chunks_by_text:
                    ...
                    logger.info(f"Incremental match: Keeping unchanged chunk at new index {idx}.")
                else:
            ...
            if obsolete_chunk_ids:
                for obsolete_id in obsolete_chunk_ids:
                    chunk_obj = db.query(DocumentChunk).filter(DocumentChunk.id == obsolete_id).first()
                    if chunk_obj:
                        db.query(VectorEmbedding).filter(VectorEmbedding.chunk_id == chunk_obj.id).delete()
                        db.delete(chunk_obj)
                logger.info(f"Incremental purge: Deleted {len(obsolete_chunk_ids)} obsolete chunks.")
            ...
            if texts_to_embed:
                ...
                if final_texts_to_embed:
                    logger.info(f"Incremental Embed: generating vectors for {len(final_texts_to_embed)} new blocks.")
                    vectors = embedding_service.get_embeddings_with_retry(final_texts_to_embed)
            ...
            db.rollback()
            logger.exception(f"Incremental reindexing failed for Document ID {document_id}:")
            doc.processing_status = "failed"
            db.commit()
        finally:
            db.close()
