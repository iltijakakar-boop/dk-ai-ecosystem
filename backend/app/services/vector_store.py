...
class SQLiteVectorStore(BaseVectorStore):
    ...
    """
    def add(self, chunk_id: int, embedding: List[float], metadata: Dict[str, Any]) -> None:
        # SQLite persistence is handled directly in indexing_service, no extra step required
        pass
        ...
        try:
            ...
            if filters:
                if "filename" in filters:
                    query = query.filter(Document.original_filename == filters["filename"])
...
class FAISSVectorStore(BaseVectorStore):
    """
    Wrapper for FAISS indexing. If faiss library is missing, delegates to SQLiteVectorStore.
    """
    def __init__(self):
        try:
            import faiss
            ...
            logger.warning("FAISS library not installed. Falling back to SQLiteVectorStore driver.")
    ...
    def search(
        ...
        # For simplicity and robust filter support, delegate search to the SQLite fallback
...
class ChromaVectorStore(BaseVectorStore):
    """
    Wrapper for ChromaDB indexing. If chromadb library is missing, delegates to SQLiteVectorStore.
    """
    def __init__(self):
        try:
            import chromadb
            ...
            logger.warning("ChromaDB library not installed. Falling back to SQLiteVectorStore driver.")
...
class VectorStoreService:
    """
    Global Vector Store routing service. Resolves provider driver dynamically based on settings.
    """
