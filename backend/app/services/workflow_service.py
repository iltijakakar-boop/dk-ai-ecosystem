...
class WorkflowService:
    ...
    matching capabilities, handling retries, pausing on
    human approvals, and DLQ routing.
    """
    ...
    def seed_default_templates(self, db: Session) -> None:
        ...
        templates = [
                    ...
                    "steps": [
                        {
                            ...
                            "input": {"query": "Ecosystem documentation trends"},
                        },
                        {
                            "name": "analysis_step",
                            "required_capability": "analysis",
                            "input": {"aspect": "modularity"},
                        },
                    ]
                },
            },
                    ...
                    "steps": [
                        {
                            ...
                            "input": {"task": "Write simple RAG context builder"},
                        },
                        {
                            "name": "analysis_step",
                            "required_capability": "analysis",
                            "input": {"rule": "Lint check code format"},
                        },
                    ]
                },
            },
            ...
            },
            {
                ...
                "description": (
                    "Pipeline requiring manual human" " confirmation to proceed"
                ),
                "definition": {
                    "steps": [
                        {
                            ...
                            "input": {"strategy": "Q2 RAG expansions"},
                        },
                        {
                            "name": "human_approval_step",
                            "required_capability": "human_approval",
    ...
    def create_or_update_workflow(self, db: Session, payload: Any) -> Workflow:
        """
        Workflow Versioning: Creates a new workflow record
        or increments version on update.
        """
        ...
        active_wf = (
            db.query(Workflow)
            .filter(Workflow.workflow_id == workflow_id, Workflow.is_active)
            .first()
        )
    ...
    def run_workflow_pipeline(self, execution_id: int) -> None:
        ...
        event_bus.publish(
            ...
            None,
            "WorkflowStarted",
            f"Workflow execution {execution_id} started.",
        )
        ...
        try:
            ...
            for step in steps:
                ...
                if exec_obj.status in ["cancelled", "failed"]:
                    logger.info(
                        f"Execution {execution_id} halted"
                        f" because status is {exec_obj.status}"
                    )
                    break

                # 1. Check if this step has already been
                # completed in a previous paused run
                existing_task = (
                    db.query(Task)
                    .filter(
                ...
                if existing_task and existing_task.status == "completed":
                    logger.info(
                        f"Step '{step_name}' already completed"
                        f" in previous run. Skipping."
                    )
                    continue
                ...
                if not existing_task:
                    existing_task = Task(
                        ...
                        max_retries=step.get(
                            "max_retries", settings.MAX_WORKFLOW_RETRIES
                        ),
                        timeout_seconds=step.get(
                            "timeout_seconds", settings.DEFAULT_TASK_TIMEOUT_SECONDS
                        ),
                    )
                ...
                event_bus.publish(
                    ...
                    task.id,
                    "TaskCreated",
                    f"Task '{step_name}' registered.",
                )

                # 4. Enforce Human-in-the-Loop Approval Check
                if (
                    step.get("requires_approval")
                    or task.required_capability == "human_approval"
                ):
                    task.status = "waiting"
                    ...
                    # Pause loop. Resuming is handled via
                    # the approval services resume endpoint
                    break

                # 5. Resolve capability matching agent
                required_cap = task.required_capability or "chat"
                assigned_agent_id = agent_orchestrator.find_agent_by_capability(
                    required_cap
                )
                ...
                if not assigned_agent_id:
                    ...
                    error_msg = (
                        f"No eligible agent found possessing"
                ...
                task_exec = TaskExecution(
                    task_id=task.id, agent_id=assigned_agent_id, status="running"
                )
                db.add(task_exec)
                db.commit()
                ...
                # Fetch inputs from shared context if referenced
                ...
                for attempt in range(1, task.max_retries + 1):
                    try:
                        ...
                        task.output_data = json.dumps(
                            {
                                "result": (
                                    f"Step '{step_name}' successfully"
                                    f" executed by {assigned_agent_id}."
                                ),
                        ...
                        shared_memory.set_variable(
                            f"{step_name}_output", json.loads(task.output_data)
                        )
                        ...
                        last_error = err
                        logger.warning(
                            f"Attempt {attempt} failed for task '{step_name}': {err}"
                        )
                ...
                if not success:
                    ...
                    error_reason = (
                        f"Task failed after {task.max_retries}"
                        f" attempts. Error: {last_error}"
                    )
            ...
            logger.exception(
                f"Unhandled crash inside workflow execution {execution_id}:"
            )
            exec_obj.status = "failed"
            db.commit()
            event_bus.publish(
                execution_id, None, "WorkflowFailed", f"Workflow execution failed: {e}"
            )
        finally:
            db.close()
    ...
    def _route_to_dlq(
        ...
        """
        Updates task and execution statuses to failed,
        and registers diagnostics logs in the DLQ.
        """
        task.status = "failed"
        task.error = reason
        exec_obj.status = "failed"
