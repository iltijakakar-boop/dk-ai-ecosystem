...
from app.schemas.document import ProviderHealthResponse, VectorStatisticsResponse
...
class SearchRequest(BaseModel):
    ...
    filters: Optional[Dict[str, Any]] = Field(
        None, example={"filename": "quarterly_report.pdf", "document_type": "pdf"}
    )
...
def query_similarity_search(payload: SearchRequest):
    ...
    results = search_service.search_similarity(
        query_text=payload.query, top_k=top_k, filters=payload.filters
    )
    return APIResponse(
        success=True, data=results, message="Similarity query execution successful."
    )
...
def query_vector_search(payload: SearchRequest, db: Session = Depends(get_db)):
    ...
    for match in matches:
        ...
        match_copy["embedding_provider"] = (
            embedding_rec.embedding_provider if embedding_rec else "unknown"
        )
        results.append(match_copy)

    return APIResponse(
        success=True, data=results, message="Vector query execution successful."
    )
...
def check_providers_health():
    ...
    return APIResponse(
        success=success, data=health_data, message="Search provider health checked."
    )
