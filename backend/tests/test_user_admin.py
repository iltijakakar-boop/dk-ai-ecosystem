...
def test_user_pagination_and_search(client: TestClient):
    ...
    client.post("/api/v1/auth/register", json={"email": "alpha@example.com", "password": "SecretPassword@123"})
    client.post("/api/v1/auth/register", json={"email": "beta@example.com", "password": "SecretPassword@123"})
...
def test_role_hierarchy_controls(client: TestClient):
    client.post("/api/v1/auth/register", json={"email": "hierarchyuser@example.com", "password": "SecretPassword@123"})
    payload_user = {
...
def test_soft_delete(client: TestClient):
    ...
    reg = client.post("/api/v1/auth/register", json={"email": "delete_me@example.com", "password": "SecretPassword@123"})
    user_id = reg.json()["id"]
