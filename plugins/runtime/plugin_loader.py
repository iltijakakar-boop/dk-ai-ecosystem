            ...
            if entry.is_dir() and not entry.name.startswith((".", "__", "runtime")):
                ...
                if os.path.exists(manifest_path):
                    try:
                        ...
                        if not plugin_id:
                            logger.error(
                                f"Plugin manifest in directory {entry.name} "
                                "is missing 'id'."
                            )
                            continue
                        ...
                        if plugin_id in self.loaded_plugins:
                            logger.error(
                                f"Duplicate plugin found! Plugin ID '{plugin_id}' "
                                "already registered."
                            )
                            continue
                        ...
                        if not manifest.get("enabled", True):
                            logger.info(
                                f"Plugin '{plugin_id}' is disabled in manifest."
                            )
                            continue
                        ...
                        logger.info(
                            f"Plugin '{plugin_id}' loaded successfully with "
                            f"{tools_count} tools."
                        )

                    except Exception:
                        logger.exception(
                            f"Failed to load plugin from folder {entry.name}:"
                        )
